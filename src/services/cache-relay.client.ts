...
export const cacheRelay$ = new BehaviorSubject<
	AbstractRelay | CacheRelay | WasmRelay | AbstractRelay | null
>(null);

// create a new cache relay instance when the url changes
localSettings.cacheRelayURL
...
cacheRelay$.pipe(pairwise()).subscribe(([prev, _current]) => {
	if (prev) {
