...
class RelayScoreboardService {
	...
	prune() {
		...
		for (const [_relay, measure] of this.relayResponseTimes)
			measure.prune(cutOff);
		for (const [_relay, measure] of this.relayEjectTime) measure.prune(cutOff);
		for (const [_relay, measure] of this.relayConnectionTime)
			measure.prune(cutOff);
	}
...
export interface RelayScoreboardManagement {
	service: RelayScoreboardService;
	startAutoSave: () => void;
	stopAutoSave: () => void;
}

export default async function createRelayScoreboardManagement(
	databaseManagement: DatabaseManagement,
): Promise<RelayScoreboardManagement> {
	const service = new RelayScoreboardService();
	const db = await databaseManagement.database;

	// Load stats on initialization
	await service.loadStats(db);
	...
	if (import.meta.env.DEV) {
		...
		window.relayScoreboardService = service;
	}
