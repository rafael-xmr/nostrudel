...
import createSecureStorage from "../classes/encrypted-storage";
...
export interface DecryptionCacheManagement {
	decryptionCache$: Observable<any>;
	decryptionCacheStats$: Observable<{
		totalEntries: number;
...
export default function createDecryptionCacheManagement(
	...
	const decryptionCache$ = enableDecryptionCache.pipe(
		switchMap((enable) => {
			if (!enable) return of(null);

			// If enabled create a database instance
			return encryptDecryptionCache.pipe(
				map((encrypt) => {
					...
					if (encrypt) {
						...
						return createSecureStorage(kv, salt);
					} else {
						return kv;
					}
				}),
				switchMap((cache) => {
					if (cache && "unlocked" in cache) {
	...
	const decryptionCacheStats$ = encryptDecryptionCache.pipe(
		switchMap(() => decryptionCache$),
		defined(),
		switchMap(async (cache) => {
			// Get the underlying localforage instance
			const kv = cache && "database" in cache ? cache.database : cache;
			...
			const isEncrypted = cache && "unlocked" in cache;
			const isLocked = cache && "unlocked" in cache ? !cache.unlocked : false;
	...
	async function autoDecryptMessagesFallback(event: NostrEvent) {
		if (autoDecryptMessages.value === false || !accounts.active) return;
		...
		if (
			...
			(event.pubkey === account.pubkey ||
				getLegacyMessageCorraspondant(event, account.pubkey) === account.pubkey)
		) {
		...
		if (
			...
			(event.pubkey === account.pubkey ||
				event.tags.find(isPTag)?.[1] === account.pubkey)
		) {
