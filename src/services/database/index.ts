...
export interface DatabaseManagement {
	database: Promise<IDBPDatabase<SchemaV13>>;
	clearCacheData: () => Promise<void>;
	deleteDatabase: () => Promise<void>;
}

export default function createDatabaseManagement(): DatabaseManagement {
	const log = logger.extend("Database");

	const dbName = "storage";
	const version = 13;

	const database = openDB<SchemaV13>(dbName, version, {
		upgrade(db, oldVersion, _newVersion, transaction, _event) {
			...
			if (oldVersion < 5) {
				...
				objectStore
					.getAll()
					.then((accounts: SchemaV4["accounts"]["value"][]) => {
