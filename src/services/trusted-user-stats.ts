...
export class TrustedUserStatsService {
	...
	async fetchUserStats(pubkey: string) {
		try {
			const stats = await fetch(
				...
				(res) =>
					res.json() as Promise<{ stats: Record<string, NostrBandUserStats> }>,
			);
	...
	async getUserStats(pubkey: string, alwaysFetch = false) {
		if (this.userStats.has(pubkey) && !alwaysFetch)
			return this.userStats.get(pubkey)!;
