...
export interface AuthenticationSignerManagement {
	relayState$: BehaviorSubject<Record<string, RelayAuthState>>;
	relayState: Record<string, RelayAuthState>;
	...
	handleRelayConnectionState: (relay: string, connected: boolean) => void;
	signEvent: <K extends number>(
		draft: Nostr.EventParameters<K>,
...
class AuthenticationSigner implements AuthenticationSignerManagement {
	...
	signEvent<K extends number>(
		...
		if (draft.kind !== kinds.ClientAuth)
			throw new Error("Event is not a client auth request");
...
export default function createAuthenticationSigner(
	...
	defaultAuthenticationMode.subscribe((mode) => {
		authenticationSigner.defaultMode = mode;
	});

	relayAuthenticationMode.subscribe((relays) => {
