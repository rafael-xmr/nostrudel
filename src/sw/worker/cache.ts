...
import type { CachedFile, CacheInfo } from "../common/interface";
...
export const getCachedFilesByCache = async (
	cacheName: string,
): Promise<CachedFile[]> => {
	const cache = await caches.open(cacheName);
	const requests = await cache.keys();
...
export const getCacheStats = async (): Promise<{
	...
	const totalFiles = cacheInfos.reduce(
...
export const refreshOfflineCache = async (): Promise<number> => {
	...
	if (!manifest || manifest.length === 0)
		throw new Error(
			"No manifest available, cannot refresh cache. This feature only works in production builds.",
		);
