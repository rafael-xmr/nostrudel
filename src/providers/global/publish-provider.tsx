...
import { BehaviorSubject, map, type Observable, share } from "rxjs";

import { scanToArray } from "../../helpers/observable";
import { useWriteRelays } from "../../hooks/use-client-relays";
import { useUserOutbox } from "../../hooks/use-user-mailboxes";
import { useLocalSettings } from "./preferences";

export type PublishResults = {
	packets: PublishResponse[];
	relays: Record<string, PublishResponse>;
};
...
export class PublishLogEntry {
	...
	constructor(
		...
		private relayPoolManagement: any,
		private eventStoreManagement: any,
	) {
		// Build a custom publish observable so the responses can be streamed back to the UI
		this.publish$ = relayPoolManagement.pool.event(relays, event).pipe(share());
		...
		this.relayStatus$ = this.results$.pipe(
			map((results) =>
				Object.fromEntries(
					relays.map((relay) => [relay, results.find((r) => r.from === relay)]),
				),
			),
		);
		...
		this.publish$.subscribe({
			next: (result) => {
				if (result.ok) {
					...
					eventStoreManagement.eventStore.update(event);
				}
			},
			complete: () => {
				this.done = true;
...
export default function PublishProvider({ children }: PropsWithChildren) {
	...
	const { relayPoolManagement, eventStoreManagement, eventCacheManagement } =
		useLocalSettings();
	...
	const publishEvent = useCallback(
		async (
			...
			try {
				...
				// sign event
				const signed = !Reflect.has(event, "sig")
					? await account!.signEvent(event)
					: (event as NostrEvent);

				const entry = new PublishLogEntry(
					label,
					signed,
					[...relays],
					relayPoolManagement,
				...
				await eventCacheManagement.writeEvent(signed);

				// add it to the event store
				eventStoreManagement.eventStore.add(signed);
