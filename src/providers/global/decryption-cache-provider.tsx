...
import { SigningService, useSigningContext } from "./signing-provider";
...
export default function DecryptionCacheServiceProvider({
	...
	const signingProvider = useSigningContext();

	const decryptionCacheService = useMemo(() => {
		if (!accountManager || signingProvider?.signingService) return undefined;

		const service = new DecryptionCache(
			accountManager,
			signingProvider?.signingService!,
		);

		return service;
	}, [accountManager, signingProvider?.signingService]);
