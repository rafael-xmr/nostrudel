...
const TrustContext = React.createContext<{
	trust: boolean;
	setOverride: (trust: boolean) => void;
}>({
	trust: false,
	setOverride: () => {},
});
...
export function TrustProvider({
	...
	const following = contactList
		? getPubkeysFromList(contactList).map((p) => p.pubkey)
		: [];

	const isEventTrusted =
		trust ||
		(!!event &&
			(event.pubkey === account?.pubkey || following.includes(event.pubkey)));
	...
	return (
		<TrustContext.Provider value={context}>{children}</TrustContext.Provider>
	);
}
