...
import { useLocalSettings } from "../global/preferences";

export default function RequireDecryptionCache({
	children,
}: {
	...
	const {
		localSettings,
		decryptionCacheManagement: { decryptionCache$, decryptionCacheStats$ },
	} = useLocalSettings();
	...
	const disableEncryption = useCallback(() => {
		...
		toast({
			...
			description:
				"The message cache encryption has been disabled and the cache has been cleared",
			status: "info",
		});
				...
				<CardBody>
					<VStack spacing="4">
						<Alert status="info" borderRadius="md">
							<Text fontSize="sm">
								The message cache is used to cache decrypted direct messages
								locally in your browser to improve loading speed. It is
								encrypted to securely store messages. Enter your password to
								unlock it and access your encrypted messages.
							</Text>
						</Alert>
						...
						<FormControl>
							...
							<Input
								...
								onKeyPress={(e) =>
									e.key === "Enter" && !unlockCache.loading && unlockCache.run()
								}
						...
						<FormControl>
							<Flex alignItems="center">
								...
								<Switch
									...
									onChange={(e) =>
										localSettings.autoDecryptMessages.next(e.target.checked)
									}
								/>
							</Flex>
							<FormHelperText>
								Automatically decrypt direct messages when they are loaded or
								received.
							</FormHelperText>
						</FormControl>
						...
						{stats && (
							<Text fontSize="sm" color="gray.500" textAlign="center">
								...
								{stats.estimatedSize > 0 &&
									` (~${Math.round(stats.estimatedSize / 1024)}KB)`}
							</Text>
						)}
					</VStack>
				</CardBody>
					...
					<VStack w="full" spacing="3">
						...
						<ButtonGroup w="full">
							<Button
								variant="ghost"
								size="sm"
								onClick={disableCacheModal.onOpen}
								colorScheme="orange"
								w="full"
							>
								Disable cache
							...
							<Button
								variant="ghost"
								size="sm"
								onClick={disableEncryptionModal.onOpen}
								colorScheme="red"
								w="full"
							>
								Disable encryption
			...
			>
				<AlertDialogOverlay>
					<AlertDialogContent>
						...
						<AlertDialogBody>
							Are you sure you want to disable message cache encryption? This
							will:
				...
				<AlertDialogOverlay>
					<AlertDialogContent>
						...
						<AlertDialogBody>
							...
							<br />• Messages will need to be decrypted every time they are
							loaded
						</AlertDialogBody>
