...
function ServerBlobStatus({
	...
	const check = reactUse.useAsync(
		() => BlossomClient.hasBlob(server, blob),
		[server, blob],
	);
...
function BlobVerificationCard({ url, hash }: { url: string; hash: string }) {
	...
	const matches = reactUse.useAsync(async () => {
...
function RepairBlobButton({
	...
	pubkey,
	...props
}: { url: string; hash: string; pubkey: string } & Omit<
	ButtonProps,
...
>) {
	...
	const repair = useAsyncAction(async () => {
		...
		const result = await multiServerUpload(userServers, blob, {
			onAuth: (_server, blob, type) =>
				createUploadAuth(async (e) => account.signEvent(e), blob, { type }),
		});
	...
	return (
		<Button
			isDisabled={!account || !userServers}
			onClick={repair.run}
			isLoading={repair.loading}
			{...props}
		>
			Repair
		</Button>
...
export function BlobDetailsModal({
	...
	hash,
	...props
}: { url: string; pubkey: string; hash: string } & Omit<
	ModalProps,
	"children"
>) {
