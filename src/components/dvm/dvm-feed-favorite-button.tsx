...
import useFavoriteFeeds, {
	FAVORITE_FEEDS_IDENTIFIER,
} from "../../hooks/use-favorite-feeds";
...
export default function DVMFeedFavoriteButton({
	pointer,
	...props
}: { pointer: AddressPointer } & Omit<
	IconButtonProps,
	"children" | "aria-label" | "isLoading" | "onClick"
>) {
	...
	const toggle = useAsyncErrorHandler(async () => {
		...
		const draft = await factory.modifyTags(
			prev,
			isFavorite ? removeCoordinateTag(pointer) : addCoordinateTag(pointer),
		);
