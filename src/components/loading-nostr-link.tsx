...
import { type DecodeResult, encodeDecodeResult } from "applesauce-core/helpers";
...
function SearchOnRelaysModal({
	...
	onClose,
	decode,
}: Omit<ModalProps, "children"> & { decode: DecodeResult }) {
	const [loading, setLoading] = useState(false);
	const [filter, setFilter] = useState("");

	const { loadersManagement, relayPoolManagement } = useLocalSettings();

	const discoveredRelays = Object.entries(
		useObservableState(relayPoolManagement.connections$) ?? {},
	).reduce<string[]>(
		(arr, [relay, status]) => (status !== "error" ? [...arr, relay] : arr),
		[],
	);
	const [relays, actions] = reactUse.useSet<string>(
		new Set(discoveredRelays.slice(0, 4)),
	);
	...
	const searchForEvent = async () => {
		...
		switch (decode.type) {
			...
			case "note":
				loadersManagement
					.eventLoader({ id: decode.data, relays: Array.from(relays) })
					.subscribe();
				break;
			case "nevent":
				loadersManagement
					.eventLoader({ id: decode.data.id, relays: Array.from(relays) })
					.subscribe();
				break;
		}
	};
	...
	const filtered = filter
		? discoveredRelays.filter((r) => r.includes(filter))
		: discoveredRelays;
...
export default function LoadingNostrLink({ link }: { link: DecodeResult }) {
			...
			case "nevent":
				return (
					<>
						...
						{link.data.relays && link.data.relays.length > 0 && (
							<Text>Relays: {link.data.relays.join(", ")}</Text>
						)}
					</>
				);
			case "npub":
	...
	return (
		<>
			...
			{search.isOpen && (
				<SearchOnRelaysModal isOpen onClose={search.onClose} decode={link} />
			)}
		</>
	);
}
