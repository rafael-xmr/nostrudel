...
export function useImageThumbnail(src?: string, settings?: AppSettings) {
	return (src && buildImageProxyURL(src, "512,fit", settings)) ?? src;
}
...
export function EmbeddedImage({
	...
	const thumbnail = useImageThumbnail(src, settings);
	...
	const verify = useCallback(
		async (e: MouseEvent<HTMLButtonElement>) => {
			...
			try {
				...
				const downloaded = bytesToHex(
					sha256.create().update(new Uint8Array(buff)).digest(),
				);
	...
	return (
		<>
			<Button
				...
				isLoading={loading}
				colorScheme={
					matches === undefined ? undefined : matches ? "green" : "red"
				}
				{...props}
			>
...
export function renderImageUrl(match: URL) {
	...
	return (
		<ExpandableEmbed
			...
			actions={
				hash ? (
					<VerifyImageButton src={match} original={hash} zIndex={1} />
				) : undefined
			}
		>
			<EmbeddedImage
