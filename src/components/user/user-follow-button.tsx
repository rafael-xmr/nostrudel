...
function UsersLists({ pubkey }: { pubkey: string }) {
	...
	const inLists = lists.filter((list) =>
		isProfilePointerInList(list, { pubkey }),
	);
	...
	const handleChange = useAsyncErrorHandler(
		async (cords: string | string[]) => {
			...
			const addToList = lists.find(
				(list) =>
					!inLists.includes(list) && cords.includes(getEventCoordinate(list)),
			);
			...
			if (addToList) {
				...
				await actions
					.exec(
						RemoveUserFromFollowSet,
						pubkey,
						getReplaceableIdentifier(removeFromList),
					)
...
export function UserFollowButton({
	...
	const toggleFollow = useAsyncErrorHandler(async () => {
		if (isFollowing) {
			await actions
				.exec(UnfollowUser, pubkey)
				.forEach((e) => publish("Unfollow user", e));
		} else {
			await actions
				.exec(FollowUser, pubkey)
				.forEach((e) => publish("Follow user", e));
		}
