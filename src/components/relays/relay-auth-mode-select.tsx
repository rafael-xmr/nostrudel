...
import type { RelayAuthMode } from "../../services/authentication-signer";
import {
	type LocalSettingsContextType,
	useLocalSettings,
} from "~/providers/global/preferences";

export function setRelayAuthMode(
	relay: string,
	mode: RelayAuthMode | null,
	...
	if (!mode) {
		if (existing)
			localSettings.relayAuthenticationMode.next(
				modes.filter((r) => r.relay !== relay),
			);
	} else {
		if (existing)
			localSettings.relayAuthenticationMode.next(
				modes.map((r) => (r.relay === relay ? { relay, mode } : r)),
			);
		else
			localSettings.relayAuthenticationMode.next([...modes, { relay, mode }]);
	}
}
...
export default function RelayAuthModeSelect({
	...
	const defaultMode = useObservableEagerState(
		localSettings.defaultAuthenticationMode,
	);
	const relayMode = useObservableEagerState(
		localSettings.relayAuthenticationMode,
	);

	const authMode = relayMode.find((r) => r.relay === relay)?.mode ?? "";

	const setAuthMode = (mode: RelayAuthMode | "") => {
		setRelayAuthMode(relay, mode || null, localSettings);
	};
