...
export function RelayScoreBreakdown({ relay }: { relay: string }) {
	...
	return (
		<Popover>
			<PopoverTrigger>
				<Button variant="link">
					{relayScoreboardService?.getRelayScore(relay)}
				</Button>
			</PopoverTrigger>
			<PopoverContent>
				<PopoverArrow />
				...
				<PopoverBody>
					<Text>
						Avg Connect:{" "}
						{relayScoreboardService?.getAverageConnectionTime(relay).toFixed(0)}{" "}
						({relayScoreboardService?.getConnectionTimeScore(relay)} pt)
					</Text>
					<Text>
						Avg Response:{" "}
						{relayScoreboardService?.getAverageResponseTime(relay).toFixed(0)} (
						{relayScoreboardService?.getResponseTimeScore(relay)} pt)
					</Text>
					<Text>
						Avg Eject:{" "}
						{relayScoreboardService?.getAverageEjectTime(relay).toFixed(0)} (
