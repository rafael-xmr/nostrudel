...
export default function EmbeddedWikiPage({
	page,
	...props
}: Omit<CardProps, "children"> & { page: NostrEvent }) {
		...
		<Card as={LinkBox} {...props}>
			<CardHeader p="2" pb="0" display="flex" gap="2" alignItems="center">
				<Heading size="md">
					<HoverLinkOverlay
						as={RouterLink}
						to={`/wiki/page/${relayHintsManagement.getSharableEventAddress(page)}`}
					>
						{getPageTitle(page)}
					</HoverLinkOverlay>
				</Heading>
				<Text>
					by <UserLink pubkey={page.pubkey} fontWeight="bold " /> -{" "}
