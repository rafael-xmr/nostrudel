...
// 	type TextareaProps as ReactTextareaAutocompleteProps,
// 	type TriggerType,
...
const Loading: any = ({ data }) => <div>Loading</div>;
...
function useEmojiTokens() {
	...
	const { value: native } = reactUse.useAsync(
		() => import("@emoji-mart/data") as Promise<{ default: EmojiMartData }>,
	);
	...
	const [frequently] = reactUse.useLocalStorage(
	...
	return useMemo(() => {
		...
		if (frequently)
			return all.sort(
				(a, b) => (frequently[b.id] ?? 0) - (frequently[a.id] ?? 0),
			);
		else return all;
...
function useAutocompleteTriggers() {
	// const { userSearchManagement, socialGraphManagement } = useLocalSettings();
	// const directory =
	// 	useObservableState(userSearchManagement.userSearchDirectory) ?? [];
	...
	// 			else
	// 				return matchSorter(emojis, token.trim(), {
	// 					keys: ["keywords"],
	// 				}).slice(0, 10);
	// 		},
	// 		component: Item,
...
const MagicInput = forwardRef<
	HTMLInputElement,
	InputProps & { instanceRef?: LegacyRef<RefType> }
...
const MagicTextArea = forwardRef<
	HTMLTextAreaElement,
	TextareaProps & { instanceRef?: LegacyRef<RefType> }
>(({ instanceRef, ...props }, ref) => {
	const triggers = useAutocompleteTriggers();
