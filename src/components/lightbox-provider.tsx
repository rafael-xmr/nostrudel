...
export function useRegisterSlide(
	ref?: RefType,
	slide?: Slide,
...
function EventSlideHeader({
	event,
	...props
}: { event: NostrEvent } & Omit<FlexProps, "children">) {
...
`;
export function CustomSlideContainer({
	slide,
	children,
...
export function LightboxProvider({ children }: PropsWithChildren) {
	...
	const lightboxSlides = useMemo(
		() => orderedSlides.current.map((s) => s.slide),
		[orderedSlides.current, slides],
	);
