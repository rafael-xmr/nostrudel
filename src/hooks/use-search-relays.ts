...
export function useCacheRelaySupportsSearch(): boolean {
	...
	const { info } = useRelayInfo(
		eventCache?.type === "local-relay" ? LOCAL_RELAY_URL : undefined,
		true,
	);
	return eventCache
		? eventCache.type === "wasm-worker" || !!info?.supported_nips.includes(50)
		: false;
}

export default function useSearchRelays(): string[] {
	const account = useActiveAccount();
	const searchRelayList = useUserSearchRelayList(
		account && { pubkey: account.pubkey },
	);
	const searchRelays = searchRelayList
		? getRelaysFromList(searchRelayList)
