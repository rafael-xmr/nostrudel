...
export default function useThreadTimelineLoader(
	...
	const rootPointer =
		refs?.root?.e || (focusedEvent && { id: focusedEvent?.id });

	const readRelays = useMemo(
		() => unique([...relays, ...(rootPointer?.relays ?? [])]),
		[relays, rootPointer?.relays],
	);
	...
	const allEvents = useMemo(() => {
		...
		if (rootEvent && focusedEvent && rootEvent.id !== focusedEvent.id)
			arr.push(rootEvent);
		return arr;
