...
export default function useTimelineViewDates(
	...
	const [cache] = useState(
		() =>
			new Map<
				string,
				{ entry: IntersectionObserverEntry; ts: number; id: string }
			>(),
	);
	...
	useEffect(() => {
		const listener = (entities: IntersectionObserverEntry[]) => {
			...
			for (const entry of entities) {
				...
				if (details?.id && details?.ts)
					cache.set(details.id, {
						entry,
						ts: parseInt(details.ts),
						id: details.id,
					});
			}
