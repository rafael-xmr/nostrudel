...
import {
	type Thread,
	useThreadsContext,
} from "../../../providers/local/thread-provider";
...
function MessagePreview({
	message,
	...props
}: { message: NostrEvent } & Omit<TextProps, "children">) {
	return (
		<DecryptPlaceholder
			message={message}
			variant="link"
...
export default function ThreadDrawer({
	...
	const decryptAll = async () => {
		...
		const promises = thread.messages
			.map((message) => {
				...
				if (container?.plaintext.value === undefined)
		...
		if (thread.root) {
			...
			if (rootContainer?.plaintext.value === undefined)
				decryptionCacheService?.requestDecrypt(rootContainer!);
		}
	...
	const renderContent = () => {
		...
		if (!thread) {
			return (
				<ThreadMessages
					thread={{ rootId: threadId, messages: [], root: getRoot(threadId) }}
					pubkey={pubkey}
				/>
			);
		}
