...
export default function ReadAuthRequiredAlert({
	relays,
	...props
}: { relays: string[] } & AlertProps) {
		...
		useObservableMemo(
			() =>
				combineLatest(
					Object.fromEntries(
						relays.map((url) => {
							const relay = relayPoolManagement.pool.relay(url);
		...
		<Alert status="warning" variant="subtle" flexShrink={0} {...props}>
			<Flex align="center" width="full" flexWrap="wrap">
				<Box overflow="hidden" isTruncated>
					<Text>
						{authRequiredRelays.length} relay
						{authRequiredRelays.length !== 1 ? "s" : ""} require
						{authRequiredRelays.length === 1 ? "s" : ""} authentication to read
						messages.
					</Text>
				...
				<ButtonGroup ms="auto" size="sm">
					{authenticateRelays.loading === false && (
						<Button
							variant="ghost"
							onClick={() => authenticateRelays.run(true)}
						>
							Always
						</Button>
					)}
