...
import type { PublishLogEntry } from "../../../providers/global/publish-provider";
...
export default function SendingStatus({
	...
	const allResults = useObservableMemo(
		() =>
			entries.length > 0
				? combineLatest(entries.map((entry) => entry.results$))
				: undefined,
		[entries],
	);
	...
	const allRelays = useMemo(
		() => mergeRelaySets(...entries.map((e) => e.relays)),
		[entries],
	);
	...
	const progress =
		totalRelays > 0
			? (successfulRelays.length + failedRelays.length) / totalRelays
			: 0;
	...
	const statusMessage = useMemo(() => {
		...
		if (allDone) {
			if (successfulRelays.length > 0) {
				const messageCount =
					entries.length > 1 ? `${entries.length} messages` : "message";
