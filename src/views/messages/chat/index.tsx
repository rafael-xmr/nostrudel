...
function DirectMessageChatPage({ pubkey }: { pubkey: string }) {
	...
	const { userDataManagement } = useLocalSettings();
	...
	const allReadRelays = useMemo(
		() => mergeRelaySets(legacyInboxes, inboxes),
		[legacyInboxes, inboxes],
	);

	const legacyMessages = useEventModel(LegacyMessagesGroup, [
		account.pubkey,
		pubkey,
	]);
	const wrappedMessages = useEventModel(WrappedMessagesGroup, [
		account.pubkey,
		pubkey,
	]);
		...
		<IntersectionObserverProvider callback={callback}>
			...
			>
				...
				<SendMessageForm
					...
					p="2"
					initialType={
						lastReceivedMessage?.kind === kinds.EncryptedDirectMessage
							? "nip04"
							: "nip17"
					}
