...
export default function DirectMessageRelayConnectionsButton({
	...
	const legacyRelays = useMemo(
		() => mergeRelaySets(selfInboxes, otherInboxes),
		[selfInboxes, otherInboxes],
	);
	const messageInboxes = useEventModel(DirectMessageRelays, [account.pubkey]);
	const allRelays = useMemo(
		() => mergeRelaySets(legacyRelays, messageInboxes),
		[legacyRelays, messageInboxes],
	);
	...
	const color = useMemo(() => {
		...
		if (allRelays.some((url) => connections[url] === "connected") === false)
			return "yellow";
