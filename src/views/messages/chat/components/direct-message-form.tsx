				...
				<ModalContent>
						...
						{value === "nip04" ? (
							<>
								<Alert status="warning" mb={4}>
									<Text>
										If the other user does not have a compatible app, they won't
										see the messages when you switch to private messaging.
									</Text>
								</Alert>
								<Text>
									Private messaging (NIP-17) provides better privacy by hiding
									message metadata from third parties.
								</Text>
							</>
						) : (
							<>
								<Alert status="warning" mb={4}>
									<Text>
										If you switch to legacy messaging, third parties will be
										able to see who you are messaging and how many messages you
										send, even though the content of the messages is encrypted.
									</Text>
								</Alert>
								<Text>
									Legacy messaging (NIP-04) is less private but more widely
									supported across different apps.
								</Text>
							</>
						)}
					</ModalBody>
					<ModalFooter p="4">
						...
						<Button
							...
							onClick={() =>
								handleTypeChange(value === "nip04" ? "nip17" : "nip04")
							}
						>
							{value === "nip04"
...
export default function SendMessageForm({
	...
	const { localSettings, eventStoreManagement, loadersManagement } =
		useLocalSettings();
	const defaultMessageExpiration = useObservableEagerState(
		localSettings.defaultMessageExpiration,
	);
	...
	const clearCache = useCacheForm<{ content: string }>(
		`dm-${pubkey}`,
		getValues,
		reset,
		formState,
	...
	const inboxes = useEventModel(GroupMessageInboxes, [
	...
	const sendMessage = handleSubmit(async (values) => {
		...
		if (type === "nip04") {
			...
			for (const event of events) {
				publishes.push(
					await publish(
						"Send message",
						event,
						mergeRelaySets(otherInboxes, selfInboxes),
						false,
						true,
					),
				);
			...
			const events = await lastValueFrom(
				actions
					.exec(SendWrappedMessage, [account.pubkey, pubkey], values.content, {
						expiration: expirationTimestamp,
					})
					.pipe(toArray()),
			);
			for (const e of events) {
				...
				const profile = eventStoreManagement.eventStore.getReplaceable(
					kinds.Metadata,
					pubkey,
				);
