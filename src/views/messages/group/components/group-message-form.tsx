...
function GroupMessageTypeButton({
	...props
}: Omit<IconButtonProps, "children" | "aria-label" | "colorScheme">) {
				...
				<ModalContent>
					...
					<ModalBody px="4" pb="2" pt="0">
						<Text>
							...
							private direct messages. which are encrypted and hide the sender
							and receiver from third parties.
						</Text>
					</ModalBody>
				</ModalContent>
			</Modal>
...
export default function GroupMessageForm({
	...
	initialExpiration,
	...props
}: { group: string; initialExpiration?: number } & Omit<
	FlexProps,
	"children"
>) {
	...
	const sendMessage = handleSubmit(async (values) => {
		...
		const events = await lastValueFrom(
			actions
				.exec(SendWrappedMessage, pubkeys, values.content, {
