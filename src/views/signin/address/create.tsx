...
function ProviderCard({
	onClick,
	provider,
}: { onClick: () => void; provider: NostrEvent }) {
...
export default function LoginNostrAddressCreate() {
	...
	const createAccount: React.FormEventHandler<HTMLDivElement> = async (e) => {
		...
		try {
			...
			if (!identity) throw new Error("Failed to fetch identity");

			if (identity.status === IdentityStatus.Error)
				throw new Error("Failed to fetch identity");
			if (identity.status === IdentityStatus.Missing)
				throw new Error("Cant find identity");
			if (identity.pubkey !== selected.pubkey)
				throw new Error("Invalid provider");

			if (identity.name !== "_")
				throw new Error("Provider does not own the domain");
			if (!identity.hasNip46)
				throw new Error("Provider does not support NIP-46");
			const relays = mergeRelaySets(
				identity.nip46Relays || identity.relays || [],
			);
			...
			const createPromise = signer.createAccount(
				name,
				identity.domain,
				undefined,
				NOSTR_CONNECT_PERMISSIONS,
			);
			await createPromise;
			...
			return (
				<>
					<div>
						...
						<Link
							href={providerMetadata?.website || userMetadata?.website}
							isExternal
							color="blue.500"
						>
