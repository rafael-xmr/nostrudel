...
import {
	type Identity,
	IdentityStatus,
} from "applesauce-loaders/helpers/dns-identity";
...
export default function LoginNostrAddressView() {
	...
	useDebounce(
		async () => {
			...
			setRootNip05(
				(await dnsIdentityLoader?.fetchIdentity("_", domain)) ?? null,
			);
		},
		300,
		[address],
	...
	const connect: React.FormEventHandler<HTMLDivElement> = async (e) => {
		...
		try {
			if (nip05.hasNip46 && nip05.pubkey) {
				...
				const account = new ReadonlyAccount(
					nip05.pubkey,
					new ReadonlySigner(nip05.pubkey),
				);
				manager.addAccount(account);
				manager.setActive(account);
