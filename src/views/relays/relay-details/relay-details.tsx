...
function getMinMaxTime(events: NostrEvent[], timeBlock = 60 * 60) {
	...
	for (const event of events) {
		if (event.created_at < minDate)
			minDate = Math.floor(event.created_at / timeBlock) * timeBlock;
		if (event.created_at > maxDate)
			maxDate = Math.ceil(event.created_at / timeBlock) * timeBlock;
	}
...
function buildLineChartData(
	...
	for (const [kind, eventsByKind] of Object.entries(byKind)) {
		const byTime: Record<number, NostrEvent[]> = groupByTime(
			eventsByKind,
			timeBlock,
		).reduce((dir, group) => ({ ...dir, [group[0]]: group[1] }), {});
...
export default function RelayDetailsTab({ relay }: { relay: string }) {
	...
	const loader = useMemo(
		() => new RelayTimelineLoader(rxNostr!, relay, [{}], { limit: 500 }),
		[rxNostr, relay],
	);
