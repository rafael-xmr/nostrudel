...
function NameForm() {
	const { webRtcRelayManagement } = useLocalSettings();
	...
	const { value: pubkey } = useAsync(async () =>
		webRtcRelayManagement.broker.signer.getPublicKey(),
	);
	const metadata = useUserProfile(pubkey);
	useEffect(() => {
		if (metadata?.name)
			reset({ name: metadata.name }, { keepDirty: false, keepTouched: false });
	}, [metadata?.name]);

	const submit = handleSubmit(async (values) => {
		const event = await webRtcRelayManagement.broker.signer.signEvent({
			kind: kinds.Metadata,
	...
	return (
		<Flex direction="column" gap="2" as="form" onSubmit={submit}>
			<FormControl isRequired>
				...
				<FormHelperText>
					The name that will be shown to other peers
				</FormHelperText>
			</FormControl>
		</Flex>
	);
	...
	return (
		<SimpleView title="Pair with WebRTC relay">
			<Text fontStyle="italic" mt="-2">
				Share this URI with other users to allow them to connect to your local
				relay
			</Text>
			...
			<Flex gap="2" alignItems="center">
				...
				<IconButton
					icon={<QrCodeIcon boxSize="1.5em" />}
					aria-label="Show QR Code"
					onClick={showQrCode.onToggle}
				/>
			...
			{webRtcRelayManagement.service.pendingIncoming.length > 0 ? (
				webRtcRelayManagement.service.pendingIncoming.map((event) => (
					<Flex
						key={event.id}
						borderWidth="1px"
						rounded="md"
					...
					>
						...
						<Button
							...
							colorScheme="green"
							onClick={() => {
								webRtcRelayManagement.acceptCall(event);
								update();
							}}
						>
							Accept
