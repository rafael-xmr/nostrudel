...
function EditWikiPagePage({ page }: { page: NostrEvent }) {
	...
	const {
		...
		defaultValues: {
			...
			title: getPageTitle(page) ?? topic,
			summary: getPageSummary(page, false),
		},
		mode: "all",
	});
	...
	const submit = handleSubmit(async (values) => {
		try {
			...
			clearFormCache();
			navigate(
				`/wiki/page/${relayHintsManagement.getSharableEventAddress(pub.event)}`,
				{ replace: true },
			);
		} catch (error) {
...
export default function EditWikiPageView() {
	...
	const page = useReplaceableEvent({
		kind: WIKI_PAGE_KIND,
