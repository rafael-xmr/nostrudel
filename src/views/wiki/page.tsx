...
function WikiPageFooter({ page }: { page: NostrEvent }) {
	...
	const subject = useMemo(
		() => dictionaryService.requestTopic(topic, readRelays),
		[topic, readRelays],
	);
	...
	let forks = pages
		...
		: [];
	if (webOfTrust)
		forks = webOfTrust.sortByDistanceAndConnections(forks, (p) => p.pubkey);
	...
	let other = pages
		...
		: [];
	if (webOfTrust)
		other = webOfTrust.sortByDistanceAndConnections(other, (p) => p.pubkey);
