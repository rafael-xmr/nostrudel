...
export default function WikiTopicView() {
	...
	const { localSettings, socialGraphManagement } = useLocalSettings();

	if (!topic) return <Navigate to="/wiki" />;

	const readRelays = useReadRelays();
	const pages = useWikiPages(topic, localSettings.readRelays, true);

	let sorted = pages ? Array.from(pages.values()) : [];
	sorted = socialGraphManagement.sortByDistanceAndConnections(
		sorted,
		(p) => p.pubkey,
	);
	...
	const { value: selected, setValue: setSelected } =
		useRouteSearchValue("pubkey");
	const selectedPage: NostrEvent | undefined =
		sorted.find((p) => p.pubkey === selected) || sorted[0];
		...
		<VerticalPageLayout>
			...
			<Flex gap="2" overflow="auto">
				{sorted.map((page, i) => (
					<Button
						...
						pr="4"
						colorScheme={
							(!selected && i === 0) || selected === page.pubkey
								? "primary"
								: undefined
						}
			...
			{sorted.length === 0 && (
				<Heading mx="auto" size="md" mt="8">
					...
					<Link
						as={RouterLink}
						to={{ pathname: "/wiki/create", search: "topic=" + topic }}
						color="blue.500"
					>
						Create one?
					</Link>
