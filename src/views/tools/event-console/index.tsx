...
export default function EventConsoleView() {
	...
	const { eventStoreManagement, relayPoolManagement, eventCacheManagement } =
		useLocalSettings();
	...
	const loadEvents = useCallback(async () => {
		try {
			...
			setLoading(true);
			setHistory((arr) =>
				arr ? (!arr.includes(query) ? [query, ...arr] : arr) : [query],
			);
			setEvents([]);
			setError("");
			...
			const handleEvent = (event: NostrEvent) => {
				event = eventStoreManagement.eventStore.add(event) ?? event;
				buffer.push(event);
				flush();
			};
			...
			if (queryRelay.isOpen) {
				...
				// query remote relay
				const sub = relayPoolManagement.pool
					.subscription([relay], filter)
					.pipe(onlyEvents())
					.subscribe(handleEvent);
				setSub(sub);
			} else {
				// query cache relay
				const sub = eventCacheManagement
					.getEvents([filter])
					.subscribe(handleEvent);
				setSub(sub);
			}
	...
	const downloadEvents = () => {
		...
		const file = new File([lines], "events.json", {
	...
	return (
		...
		>
			<Flex gap="2" wrap="wrap" alignItems="center">
				<Switch
					size="sm"
					isChecked={queryRelay.isOpen}
					onChange={queryRelay.onToggle}
				>
					Query Relay
				</Switch>
				{queryRelay.isOpen && (
