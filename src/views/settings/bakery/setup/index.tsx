...
export default function BakerySetupView() {
	...
	const { bakery$, controlApi$, setBakeryURL } = useBakeryProvider();
			...
			if (!pubkey && values.owner.includes("@")) {
				try {
					...
					if (identity?.status !== IdentityStatus.Found)
						throw new Error("Missing pubkey in NIP-05");
	...
	if (relayParam.value) {
		if (bakery) {
			if (
				new URL(bakery.url).toString() !== new URL(relayParam.value).toString()
			) {
		...
		<Flex w="full" h="full" alignItems="center" justifyContent="center">
			...
			>
				...
				<FormControl isRequired>
					...
					<FormHelperText>
						Enter the NIP-05, npub, or hex pubkey of the owner of this node
					</FormHelperText>
				</FormControl>
				<Button
					type="submit"
