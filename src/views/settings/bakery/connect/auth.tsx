...
export function BakeryAuthPage() {
	...
	const { bakery$, setBakeryURL } = useBakeryProvider();
	...
	const authenticate = async (
		auth: string | ((evt: EventTemplate) => Promise<VerifiedEvent>),
	) => {
		if (!bakery) return;
	...
	const authenticateWithNostr = async () => {
		try {
			...
			await authenticate(
				(draft) => requestSignature(draft) as Promise<VerifiedEvent>,
			);
		} catch (error) {
	...
	const submit = handleSubmit(async (values) => {
		if (remember.isOpen)
			localStorage.setItem("personal-node-auth", values.auth);
				...
				{formState.isSubmitting ? (
					...
					<>
						...
						<Flex mt="2" justifyContent="space-between">
							<Checkbox
								isChecked={remember.isOpen}
								onChange={remember.onToggle}
							>
								Remember Me
							</Checkbox>
