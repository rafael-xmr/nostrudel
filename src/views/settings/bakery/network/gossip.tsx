...
function AddRelayForm() {
	...
	const submit = handleSubmit((values) => {
		...
		controlApi?.setConfigField("gossipBroadcastRelays", [
	...
	return (
		<>
			<Flex alignItems="center" gap="2">
				...
				<Link
					isExternal
					href="https://github.com/dskvr/nips/blob/nip-59-relay-status/66.md"
					color="GrayText"
					ml="auto"
				>
					More Info
			...
			{config?.gossipEnabled && (
				<>
					...
					<Flex direction="column" gap="2">
						{config?.gossipBroadcastRelays.map((relay) => (
							<BroadcastRelay key={relay} relay={relay} />
						))}
					</Flex>
				</>
			)}
