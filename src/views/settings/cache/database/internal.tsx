...
async function importEvents(events: NostrEvent[], database: NostrIDB) {
...
async function exportEvents(database: NostrIDB) {
	return (await database.getAll("events")).map((row) => row.event);
}

export default function InternalDatabasePage() {
	const {
		databaseManagement: { clearCacheData, deleteDatabase },
		indexedDBManagement: { database },
	} = useLocalSettings();
	...
	return (
		<>
			...
			<ButtonGroup flexWrap="wrap">
				<ImportEventsButton onLoad={(e) => importEvents(e, database)} />
				<ExportEventsButton getEvents={() => exportEvents(database)} />
			</ButtonGroup>
			<ButtonGroup flexWrap="wrap">
				<Button
