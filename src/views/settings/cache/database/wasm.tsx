...
export default function WasmDatabasePage() {
	const { localSettings, eventCacheManagement, wasmWorkerManagement } =
		useLocalSettings();
	const eventCache = useObservableEagerState(eventCacheManagement.eventCache$);
	if (eventCache?.type !== "wasm-worker")
		return <Navigate to="/settings/cache" />;

	const worker = wasmWorkerManagement.worker;

	const [summary, setSummary] = useState<Record<string, number>>();

	const persistForDays = useObservableEagerState(
		localSettings.wasmPersistForDays,
	);

	const total = summary
		? Object.values(summary).reduce((t, v) => t + v, 0)
		: undefined;
	...
	return (
		<>
			...
			<ButtonGroup flexWrap="wrap">
				...
				<Button
					colorScheme="red"
					onClick={deleteDatabase.run}
					isLoading={deleteDatabase.loading}
				>
					Delete database
				</Button>
			</ButtonGroup>
