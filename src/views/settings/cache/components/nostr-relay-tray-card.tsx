...
export default function NostrRelayTrayCard() {
	...
	const cacheRelay = useObservableEagerState(eventCacheManagement.eventCache$);
	...
	const enable = useAsyncAction(async () => {
		await eventCacheManagement.changeEventCache("local-relay");
	});
	...
	return (
		<Card borderColor={enabled ? "primary.500" : undefined} variant="outline">
			...
			<CardBody p="4" pt="0">
				<Text mb="2">
					A cool little app that runs a local relay in your systems tray
				</Text>
				<Text>Maximum capacity: Unlimited</Text>
				<Text>Performance: As fast as your computer</Text>
				{!available && (
					<Text color="yellow.500">
						If the app is running and the button still says "Get the app" the
						browser is probably blocking access to the relay
					</Text>
				)}
			</CardBody>
		</Card>
