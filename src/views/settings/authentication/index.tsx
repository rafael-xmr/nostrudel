...
export default function AuthenticationSettingsView() {
	const { localSettings, relayPoolManagement } = useLocalSettings();

	const connections =
		useObservableState(relayPoolManagement.connections$) ?? {};
	const sortedRelays = Object.keys(connections).sort();

	const proactivelyAuthenticate = useObservableEagerState(
		localSettings.proactivelyAuthenticate,
	);
			...
			<FormControl>
				<Flex alignItems="center">
					...
					<Switch
						...
						onChange={(e) =>
							localSettings.proactivelyAuthenticate.next(
				...
				<FormHelperText>
					<span>
						Authenticate to relays as soon as they send the authentication
						challenge
					</span>
				</FormHelperText>
			</FormControl>
