...
function MediaServersPage() {
	...
	const { event, servers } = useUsersMediaServers(
		account.pubkey,
		undefined,
		true,
	);
	...
	const removeServer = async (server: string) => {
		...
		draft.tags = [
			...draft.tags.filter((t) => !isServerTag(t)),
			...servers
				.filter((s) => !areServersEqual(s, server))
				.map((server) => ["server", server.toString()]),
		];
			...
			{mediaUploadService !== "blossom" && (
				<Alert status="info">
					...
					<Box>
						...
						<AlertDescription>
							These servers wont be used for anything unless you set "Media
							upload service" to "Blossom" in the settings
						</AlertDescription>
						<br />
			...
			{servers.length === 0 && mediaUploadService === "blossom" && (
				...
				>
					...
					<AlertDescription maxWidth="sm">
						You need to add at least one media server in order to upload images
						and videos
					</AlertDescription>
					<Divider maxW="96" w="full" my="2" />
					<Button onClick={() => setValue("server", "https://nostr.download/")}>
						Add nostr.download
					</Button>
				</Alert>
			)}
