...
function RelayEntry({ url, onRemove }: { url: string; onRemove: () => void }) {
	...
	const authRequiredForPublishing = useObservableMemo(
		() => pool.relay(url).authRequiredForPublish$,
		[url],
	);
	const authRequiredForReading = useObservableMemo(
		() => pool.relay(url).authRequiredForRead$,
		[url],
	);
				...
				<>
					...
					{authRequiredForPublishing && (
						<Text color="red.500" fontSize="sm">
							Authentication required for publishing (This may prevent users
...
export default function DirectMessageRelaysSection() {
	...
	const dmRelayList = useAddressableEvent(
		account
			? { kind: kinds.DirectMessageRelaysList, pubkey: account.pubkey }
			: undefined,
	);
		...
		<VStack spacing="4" align="stretch">
			<VStack spacing="1" align="start">
				...
				<Text fontSize="sm" color="gray.500">
					Relays where your encrypted direct messages will be sent and received
					according to{" "}
					<Link
						color="blue.500"
						isExternal
			...
			<VStack spacing="2" align="stretch">
				{relays.map((relay) => (
					<RelayEntry
						key={relay}
						url={relay}
						onRemove={() => removeRelay.run(relay)}
					/>
				))}
				{relays.length === 0 && (
					<Alert status="warning" variant="subtle">
						<AlertIcon />
						<AlertDescription>
							No direct message relays configured. Add relays below to enable
