...
function DecryptionCacheStats() {
	const { decryptionCacheManagement } = useLocalSettings();

	const stats = useObservableState(
		decryptionCacheManagement.decryptionCacheStats$,
	);
	...
	const clearCache = useCallback(async () => {
		if (
			!confirm(
				"Are you sure you want to clear the message cache? This will remove all cached decrypted messages.",
			)
		) {
			return;
		}
		...
		const cache = await firstValueFrom(
			decryptionCacheManagement.decryptionCache$,
		);
		if (!cache) return;
		...
		<VStack spacing="3" align="stretch">
			<Flex justify="space-between" align="center">
				...
				<HStack spacing={2}>
					...
					{stats.isEncrypted && (
						<Badge colorScheme={stats.isLocked ? "red" : "green"}>
			...
			{stats.totalEntries > 0 ? (
				<SimpleGrid spacing={1} columns={2}>
					...
					<Text
						fontWeight="bold"
						color={stats.isEncrypted ? "green.500" : "orange.500"}
						textAlign="right"
					>
					...
					{stats.isEncrypted && (
						<>
							...
							<Text
								fontWeight="bold"
								color={stats.isLocked ? "red.500" : "green.500"}
								textAlign="right"
							>
								{stats.isLocked ? "Locked" : "Unlocked"}
							</Text>
...
export default function MessageCacheSection() {
	...
	const encryptDecryptionCache = useObservableEagerState(
		localSettings.encryptDecryptionCache,
	);
	const enableDecryptionCache = useObservableEagerState(
		localSettings.enableDecryptionCache,
	);
		...
		<VStack spacing="4" align="stretch">
			<VStack spacing="1" align="start">
				...
				<Text fontSize="sm" color="gray.500">
					Configure how decrypted messages are cached locally to improve
					performance.
				</Text>
			</VStack>
			...
			<FormControl>
				<Flex alignItems="center">
					...
					<Switch
						...
						onChange={(e) =>
							localSettings.enableDecryptionCache.next(e.target.checked)
						}
					/>
				</Flex>
				<FormHelperText>
					Store decrypted messages locally in your browser to speed up message
					loading.
				</FormHelperText>
			</FormControl>
					...
					<FormControl>
						<Flex alignItems="center">
							...
							<Switch
								...
								onChange={(e) =>
									localSettings.encryptDecryptionCache.next(e.target.checked)
								}
							/>
						</Flex>
						<FormHelperText>
							Whether to encrypt cached decrypted messages with a password when
