...
function MutedThread({
	thread,
	onRemove,
}: {
	thread: string;
	onRemove: (thread: string) => void;
}) {
...
export default function MutedThreadsSection() {
	...
	const addThread = useCallback(async () => {
		...
		try {
			...
			if (decoded.type !== "nevent")
				throw new Error("Please enter a valid nevent (starts with nevent1)");

			// Check if already muted
			if (muted?.threads.has(decoded.data.id))
				throw new Error("This thread is already muted");

			await actions
				.exec(MuteThread, decoded.data)
				.forEach((e) => publish("Add muted thread", e));
			setNewNevent("");
		} catch (e) {
	...
	const removeThread = useCallback(
		async (eventId: string) => {
			await actions
				.exec(UnmuteThread, eventId)
				.forEach((e) => publish("Remove muted thread", e));
		},
		[publish, actions],
	);
		...
		<VStack spacing={2} align="stretch">
			<Box>
				...
				<Text color="gray.500" fontSize="sm">
					Hide specific threads/events. Paste a nevent (nip-19 event identifier)
					to mute a thread.
				</Text>
			</Box>
