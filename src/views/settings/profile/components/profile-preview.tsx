...
export default function ProfilePreview() {
	...
	const pictureObjectUrl = useObjectURL(
		data.picture instanceof File ? data.picture : undefined,
	);
	const bannerObjectUrl = useObjectURL(
		data.banner instanceof File ? data.banner : undefined,
	);
						...
						<Flex align="end" mb={4}>
							...
							<VStack align="start" ml={4} spacing={1} flex={1}>
								<Text fontSize="xl" fontWeight="bold">
									{getDisplayName(
										data as ProfileContent,
										truncateId(npubEncode(account.pubkey)),
									)}
								</Text>
								{data.nip05 && (
