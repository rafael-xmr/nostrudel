...
export default function PrivacySettings() {
	...
	const { localSettings, requestProxyManagement } = useLocalSettings();
	const debugApi = useObservableEagerState(localSettings.enableDebugApi);

	const validateRequestProxy = async (url?: string) => {
		if (!url) return true;
		try {
			const controller = new AbortController();
			const timeoutId = setTimeout(() => controller.abort(), 2000);
			const res = await fetch(
				requestProxyManagement.createRequestProxyUrl(
					"https://example.com",
					url,
				),
				{
	...
	return (
		...
		>
			...
			<FormControl>
				...
				{formState.errors.shareService && (
					<FormErrorMessage>
						{formState.errors.shareService.message}
					</FormErrorMessage>
				)}
				<FormHelperText>
					A URL to an instance of{" "}
					<Link
						href="https://github.com/fiatjaf/njump"
						isExternal
						color="blue.500"
					>
						njump
