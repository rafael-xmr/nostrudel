...
function EditableIdentity() {
	...
	const onSubmit: EditableProps["onSubmit"] = async (value) => {
		if (!account) return;
		try {
			const metadata = eventStore.getReplaceable(
				kinds.Metadata,
				account.pubkey,
			);
			...
			const draft = await factory.modify(
				metadata,
				setContent(JSON.stringify(newProfile)),
			);
...
function IdentityDetails({
	pubkey,
	profile,
}: { pubkey: string; profile: ProfileContent }) {
			...
			{identity?.status === IdentityStatus.Found && (
				<>
					...
					<Text fontStyle="italic" mt="-2">
						You have {identity.relays?.length ?? 0} relays set in your DNS
						identity
					</Text>
		...
		>
			{profile?.nip05 ? (
				...
				<>
					...
					<Text>
						or read the details on the wiki:{" "}
						<WikiLink topic="nip-05">NIP-05</WikiLink>
					</Text>
				</>
			)}
		</SimpleView>
