...
function Badge({
	...
	badge,
	award,
}: { pubkey: string; badge: NostrEvent; award: NostrEvent }) {
	const relayHints = useRelayHints();
	const naddr = relayHints.getSharableEventAddress(badge);
				...
				<ModalContent>
					...
					<ModalBody px="4" py="2">
						<Text>
							...
							<UserLink fontWeight="bold" pubkey={badge.pubkey} /> on{" "}
							<Timestamp timestamp={badge.created_at} />
						</Text>
						<Text>
...
export default function UserProfileBadges({
	pubkey,
	...props
}: Omit<FlexProps, "children"> & { pubkey: string }) {
	...
	return (
		<Flex gap="2" wrap="wrap" alignItems="center" {...props}>
			{(limit !== null ? profileBadges.slice(0, limit) : profileBadges).map(
				({ badge, award }) => (
					<Badge
						key={getEventCoordinate(badge)}
						pubkey={pubkey}
						badge={badge}
						award={award}
					/>
