...
import { type Filter, kinds, nip19, type NostrEvent } from "nostr-tools";
import { Link as RouteLink, type To } from "react-router-dom";
...
import type { RelayHintsManagement } from "~/services/relay-hints";
...
function consoleLink(events: NostrEvent[], pubkey: string) {
	...
	return {
		to: "/tools/console",
		state: {
			filter: { kinds: Array.from(kinds), authors: [pubkey] } satisfies Filter,
		},
	};
}
...
const KnownKinds: KnownKind[] = [
...

	{
		...
		name: "Report",
		icon: AlertTriangle,
		link: (_e, p) => `/u/${npubEncode(p)}/reports`,
	},

	{
		...
		name: "Emojis",
		icon: EmojiPacksIcon,
		link: (_e, p) => `/u/${npubEncode(p)}/emojis`,
	},
...
function EventKindButton({
	...
	const icon = Icon ? (
		...
		<AnnotationQuestion boxSize={10} mb="4" />
	);
	...
	if (!nav) {
		if (events.length === 1) {
			nav =
				known?.single?.(events[0], pubkey) ||
				singleLink(events[0], pubkey, relayHintsManagement);
		} else {
...
export default function UserRecentEvents({ pubkey }: { pubkey: string }) {
	...
	const { timeline: recent } = useTimelineLoader(
		`${pubkey}-recent-events`,
		outbox || readRelays,
		{
			authors: [pubkey],
			limit: 100,
		...
		<Flex gap="2" wrap="wrap">
			{byKind &&
				Object.entries(byKind)
					.filter(
						([_, { known }]) =>
							(!!known || all.isOpen) && (known ? known.hidden !== true : true),
					)
			...
			{!all.isOpen && (
				<Button variant="link" p="4" onClick={all.onOpen}>
					Show more (
					{Object.entries(byKind).filter(([_, { known }]) => !!known).length})
				</Button>
			)}
		</Flex>
	);
