...
export default function TaskManagerProvider({
	...
	const openInParent = useCallback(
		(to: To) => parentRouter.navigate(to),
		[parentRouter],
	);
	...
	useEffect(() => {
		return parentRouter.subscribe((event) => {
			...
			if (subRoute) {
				if (router) {
					if (
						router.state.location.pathname !== subRoute &&
					...
					newRouter.subscribe((e) => {
						if (
							...
							parentRouter.navigate(parentRouter.state.location, {
								...
								state: {
									...parentRouter.state.location.state,
									taskManagerRoute: e.location.pathname,
								},
							});
						}
						direction.current = undefined;
	...
	const closeTaskManager = useCallback(() => {
		...
		if (i !== null && i > 0) {
			log(
				`Navigating back ${i} entries to the point the task manager was opened`,
			);
	...
	return (
		<TaskManagerContext.Provider value={context}>
			{children}
			{router && (
				<TaskManagerModal router={router} isOpen onClose={closeTaskManager} />
			)}
