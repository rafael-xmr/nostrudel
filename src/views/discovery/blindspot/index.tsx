...
const UserCard = memo(
	({ pubkey, blindspot }: { pubkey: string; blindspot: string[] }) => {
		return (
...
export default function BlindspotHomeView() {
	...
	const pubkeys = useMemo(
		() => graph.getFollowedByUser(account.pubkey),
		[graph, account.pubkey],
	);
	...
	const blindspots = useMemo(() => {
		...
		const arr = Array.from(pubkeys)
			.map((pubkey) => {
				...
				const blindspot =
					Array.from(following).filter(
						(p) => !pubkeys.has(p) && p !== account.pubkey,
					) ?? [];
		...
		if (sort === "follows") {
			...
			for (const { pubkey, blindspot } of arr) {
				const total = blindspot.reduce(
					(t, p) => t + (graph.getFollowDistance(p) ?? 0),
					0,
				);
			...
			return arr.sort(
				(a, b) => quality.get(a.pubkey)! - quality.get(b.pubkey)!,
			);
		}
	...
	return (
		<SimpleView title="Blindspots" center maxW="container.xl">
			...
			>
				<Text>
					Pick another user and see what they are seeing that your not.
				</Text>
				<Select
					ml="auto"
					maxW="48"
