...
export default function RelayDiscoveryView() {
	...
	const [discoveryRelay, setDiscoveryRelay] = useState(
	...
	const [request, setRequest] =
		useState<ReturnType<typeof createRxForwardReq>>();
	...
	useEffect(() => {
		...
		const sub = rxNostr
			.use(req, { on: { relays: [discoveryRelay] } })
			.subscribe((packet) => {
				if (getReplaceableIdentifier(packet.event)) {
					setEvents((arr) => ({
						...arr,
						[getEventUID(packet.event)]: packet.event,
					}));
				}
			});
	...
	}, [rxNostr, discoveryRelay, setEvents]);
