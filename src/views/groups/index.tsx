...
function useFriendsGroupsList() {
	...
	const { timeline: groupListEvents } = useTimelineLoader(
	...
	const groupsByPopularity = useMemo(() => {
		...
		const groupCounts = new Map<
			string,
			{ pointer: GroupPointer; users: string[] }
		>();
			...
			{hasUserGroups ? (
				...
				>
					...
					<AlertDescription maxWidth="sm">
						You haven't bookmarked any groups yet. Start by exploring groups to
...
function FriendsGroups() {
	const account = useActiveAccount();
	const { pointers: userGroups } = useUserGroupsList();
	const { groupsByPopularity: friendsGroups, loading: friendsLoading } =
		useFriendsGroupsList();
	...
	const filteredFriendsGroups =
		friendsGroups?.filter(
			(group) =>
				!userGroups.some(
					(g) => encodeGroupPointer(g) === encodeGroupPointer(group.pointer),
				) && (showSingleUserGroups ? true : group.users.length > 1),
		) || [];

	const hasFriendsGroups =
		filteredFriendsGroups && filteredFriendsGroups.length > 0;
		...
		<Box>
			...
			{!account ? (
				<Alert status="info" variant="subtle">
					...
					<AlertDescription>
						Sign in to see groups that your friends have bookmarked.
					</AlertDescription>
				</Alert>
			) : friendsLoading ? (
				<SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap="4">
				...
				>
					...
					<AlertDescription maxWidth="sm">
						None of your contacts have bookmarked any groups yet, or they
						haven't published their group lists.
					</AlertDescription>
				</Alert>
			)}
		</Box>
