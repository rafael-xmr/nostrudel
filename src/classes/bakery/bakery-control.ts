...
import type { PrivateNodeConfig } from "@satellite-earth/core/types";
import type { Relay } from "applesauce-relay";
import hash_sum from "hash-sum";

import type { LogEntry, NetworkStateResult } from "./types";
...
export default class BakeryControlApi {
	...
	constructor(public bakery: Relay) {
		this.config = this.query<PrivateNodeConfig>("config", {}).pipe(
			shareReplay(1),
		);
		this.network = this.query<NetworkStateResult>("network-status", {}).pipe(
			shareReplay(1),
		);
		this.services = this.query<{ id: string }>("services", {}).pipe(
			scan(
				(arr, service) =>
					arr.includes(service.id) ? arr : [...arr, service.id],
				[] as string[],
			),
			shareReplay(1),
		);
	...
	query<T extends unknown = unknown, R extends unknown = T>(
		...
		let query = this.bakery.socket$
			.multiplex(
				...
				() => ["QRY", "CLOSE", id],
				(m) =>
					m[0] === "QRY" && (m[1] === "DATA" || m[1] === "ERR") && m[2] === id,
			)
			.pipe(
				map((message) => {
		...
		// Apply modify with type assertion to bypass version mismatch
		if (modify) query = modify(query as any) as Observable<R>;
	...
	logs(filter: { service?: string; limit?: number }) {
		return this.query<LogEntry, LogEntry[]>("logs", filter, (source) =>
			source.pipe(scanToArray()),
		);
	}

	async setConfigField<T extends keyof PrivateNodeConfig>(
		field: T,
		value: PrivateNodeConfig[T],
	) {
