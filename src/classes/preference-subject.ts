...
export class PreferenceSubject<T = string> extends BehaviorSubject<T> {
	...
	async save(value: T = this.value) {
		...
		if (!encoded)
			throw new Error("encode can not return null when saveDefault is set");
	...
	static async create<T>(
		...
		const value = await PreferenceSubject.load(key, fallback, opts);
	...
	static async string(
		key: string,
		fallback: string,
		opts?: PreferencesSubjectOptions<string>,
	) {
	...
	static async array<T>(
		key: string,
		fallback: T[],
		opts?: Omit<PreferencesSubjectOptions<T[]>, "decode" | "encode">,
	) {
