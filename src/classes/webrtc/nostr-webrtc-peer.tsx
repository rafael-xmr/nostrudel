...
export default class NostrWebRTCPeer extends EventEmitter<EventMap> {
	...
	constructor(
		...
		this.connection.onicegatheringstatechange =
			this.flushCandidateQueue.bind(this);
	...
	private async flushCandidateQueue() {
		...
		if (
			...
			const cipherText = await this.signer.nip44.encrypt(
				this.peer,
				JSON.stringify(this.candidateQueue),
			);
		...
		this.subscription = this.pool.subscribeMany(
			...
			],
			{
				onevent: async (event: NostrEvent) => {
					...
					if (
						!event.tags.some(
							(t) => t[0] === "e" && t[1] === this.offerEvent?.id,
						)
					)
						return;
	...
	async handleAnswer(event: NostrEvent) {
		...
		const plaintext = await this.signer.nip44.decrypt(
			event.pubkey,
			event.content,
		);
		const answer = JSON.parse(plaintext) as RTCSessionDescriptionInit;
		if (answer.type !== "answer")
			throw new Error("Unexpected rtc description type");
	...
	async answerCall(event: NostrEvent) {
		...
		this.signalingRelays = event.tags
