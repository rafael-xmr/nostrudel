...
import NostrWebRTCPeer, {
	type Pool,
	RTCDescriptionEventKind,
	type Signer,
} from "./nostr-webrtc-peer";
...
export default class NostrWebRtcBroker extends EventEmitter<EventMap> {
	...
	async answerCall(event: NostrEvent): Promise<NostrWebRTCPeer> {
		if (this.peers.has(event.pubkey))
			throw new Error("Already have a peer connection for this pubkey");
	...
	static parseNostrWebRtcURI(uri: string | URL) {
		...
		if (keyParam && !isHex(keyParam))
			throw new Error("Key must be in hex format");
		const key = keyParam ? hexToBytes(keyParam) : null;
		return { pubkey, key, relays };
	}

	static createNostrWebRtcURI(
		pubkey: string,
		relays: string[],
		...
		if (key === true)
			uri.searchParams.append("key", bytesToHex(generateSecretKey()));
		else if (key instanceof Uint8Array)
			uri.searchParams.append("key", bytesToHex(key));
		return uri.toString();
	}
}
